...
#define  LEFT_RIGHT_PIXELS			BIT(7)
#define  DSI_CHANNEL_MODE_MASK			GENMASK(6, 5)
#define  DSI_CHANNEL_MODE_DUAL			0
#define  DSI_CHANNEL_MODE_SINGLE		BIT(5)
#define  CHA_DSI_LANES_MASK			GENMASK(4, 3)
#define  CHA_DSI_LANES(x)			((x) << 3)
#define  CHB_DSI_LANES_MASK			GENMASK(2, 1)
#define  CHB_DSI_LANES(x)			((x) << 1)
#define SN_DSIA_CLK_FREQ_REG			0x12
#define SN_DSIB_CLK_FREQ_REG			0x13
#define SN_CHA_ACTIVE_LINE_LENGTH_LOW_REG	0x20
#define SN_CHB_ACTIVE_LINE_LENGTH_LOW_REG	0x22
 ...
 * @host_node:    Remote DSI nodes.
 * @dsi:          Our MIPI DSI sources.
 * @dsi_lanes:    Lane count for each DSI source.
 * @dsi_count:    Number of connected DSI sources.
...
struct ti_sn65dsi86 {
	...
	struct device_node		*host_node[SN_MAX_DSI_CHANNELS];
	struct mipi_dsi_device		*dsi[SN_MAX_DSI_CHANNELS];
	int				dsi_lanes[SN_MAX_DSI_CHANNELS];
	int				dsi_count;
	struct clk			*refclk;
				     ...
				     struct drm_atomic_state *state)
{
	...
	bit_rate_khz = mode->clock *
			mipi_dsi_pixel_format_to_bpp(pdata->dsi[0]->format);
	if (pdata->dsi_count == 2)
		bit_rate_khz /= 2;

	clk_freq_khz = bit_rate_khz / (pdata->dsi[0]->lanes * 2);
...
static int ti_sn_attach_host(struct auxiliary_device *adev, struct ti_sn65dsi86 *pdata)
{
	...
	for (i = 0; i < pdata->dsi_count; i++) {
		...
		host = of_find_mipi_dsi_host_by_node(pdata->host_node[i]);
		if (!host)
			return -EPROBE_DEFER;
		...
		val = devm_mipi_dsi_attach(&adev->dev, dsi);
		if (val)
			return val;
	}
				      ...
				      struct drm_atomic_state *state)
{
	...
	clk_freq_mhz = bit_rate_mhz / (pdata->dsi[0]->lanes * 2);
	...
	if (pdata->dsi_count == 2)
		regmap_write(pdata->regmap, SN_DSIB_CLK_FREQ_REG, val);
}
					   ...
					   struct drm_atomic_state *state)
{
	...
	if (pdata->dsi_count == 2) {
		ti_sn65dsi86_write_u16(pdata, SN_CHA_ACTIVE_LINE_LENGTH_LOW_REG,
				       mode->hdisplay / 2);
		ti_sn65dsi86_write_u16(pdata, SN_CHB_ACTIVE_LINE_LENGTH_LOW_REG,
				       mode->hdisplay - mode->hdisplay / 2);
	} else {
		ti_sn65dsi86_write_u16(pdata, SN_CHA_ACTIVE_LINE_LENGTH_LOW_REG,
				       mode->hdisplay);
	}
				       ...
				       struct drm_atomic_state *state)
{
	...
	/* DSI lane and channel config */
	if (pdata->dsi_count == 2)
		val = LEFT_RIGHT_PIXELS |
		      DSI_CHANNEL_MODE_DUAL |
		      CHA_DSI_LANES(SN_MAX_DP_LANES - pdata->dsi[0]->lanes) |
		      CHB_DSI_LANES(SN_MAX_DP_LANES - pdata->dsi[1]->lanes);
	else
		val = DSI_CHANNEL_MODE_SINGLE |
		      CHA_DSI_LANES(SN_MAX_DP_LANES - pdata->dsi[0]->lanes);

	regmap_update_bits(pdata->regmap, SN_DSI_LANES_REG,
			   LEFT_RIGHT_PIXELS | DSI_CHANNEL_MODE_MASK |
			   CHA_DSI_LANES_MASK | CHB_DSI_LANES_MASK, val);
...
static int ti_sn_bridge_parse_dsi_host(struct ti_sn65dsi86 *pdata)
{
	...
	for (i = 0; i < SN_MAX_DSI_CHANNELS; i++) {
		pdata->host_node[i] = of_graph_get_remote_node(np, 0, i);
		if (!pdata->host_node[i]) {
			if (!i)
				break;
			continue;
		}

		pdata->dsi_lanes[i] = SN_MAX_DP_LANES;

		endpoint = of_graph_get_endpoint_by_regs(np, 0, i);
		if (endpoint) {
			struct device_node *remote;
			int dsi_lanes;
			bool got_lanes = false;

			dsi_lanes = drm_of_get_data_lanes_count(endpoint, 1,
								SN_MAX_DP_LANES);
			if (dsi_lanes > 0) {
				pdata->dsi_lanes[i] = dsi_lanes;
				got_lanes = true;
			}
			...
			if (!got_lanes) {
				...
				if (remote) {
					dsi_lanes = drm_of_get_data_lanes_count(remote, 1,
										SN_MAX_DP_LANES);
					if (dsi_lanes > 0)
						pdata->dsi_lanes[i] = dsi_lanes;
				}
...
static void ti_sn_bridge_remove(struct auxiliary_device *adev)
{
	...
	for (int i = 0; i < pdata->dsi_count; i++)
		of_node_put(pdata->host_node[i]);
}
